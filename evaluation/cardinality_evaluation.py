...
from evaluation.utils import parse_query, save_csv, parse_sdql_query
...
def evaluate_cardinalities(ensemble_location, physical_db_name, query_filename, target_csv_path, schema,
                           ...
                           max_variants=1, merge_indicator_exp=False, exploit_overlapping=False, min_sample_ratio=0, is_sdql=True):
    """
    ...
    if is_sdql:
        ...
        if true_cardinalities_path is not None:
            with open(true_cardinalities_path, 'rb') as file:
                lines = file.readlines()
                for line in lines:
                    line_int = int(line)
                    true_card.append(line_int)
        else:
        ...
        with open(query_filename, "r") as f:
            ...
            for line in lines:
                ...
                if "@sdql_compile" in line_str:
                    real_query.append([line_str])
                else:
                    real_query[-1].append(line_str)

        if use_generated_code:
            spn_ensemble.use_generated_code()

        latencies = []
        for query_no, query_str in enumerate(real_query):
            query_str = ' '.join([line.strip() for line in query_str])
            logger.debug(f"Predicting cardinality for query {query_no}: {query_str}")

            query, decay_factor = parse_sdql_query(query_str.strip(), schema)
            ...
            _, factors, cardinality_predict, factor_values = spn_ensemble \
                .cardinality(query, rdc_spn_selection=rdc_spn_selection, pairwise_rdc_path=pairwise_rdc_path,
                            ...
                            gen_code_stats=gen_code_stats, decay_factor=decay_factor)
            card_end_t = perf_counter()
            latency_ms = (card_end_t - card_start_t) * 1000

            logger.debug(f"\t\tLatency: {latency_ms:.2f}ms")
            logger.debug(f"\t\tTrue: {cardinality_true[query_no]}")
            logger.debug(f"\t\tPredicted: {cardinality_predict}")

            q_error = max(cardinality_predict / cardinality_true[query_no], cardinality_true[query_no] / cardinality_predict)
        ...
        for query_no, query_str in enumerate(queries):
            ...
            query, decay_factor = parse_query(query_str.strip(), schema)
            assert query.query_type == QueryType.CARDINALITY
