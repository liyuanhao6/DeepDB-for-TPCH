        ...
        for (table, condition) in conditions:
            ...
            if self.meta_types[attribute_index] == MetaType.DISCRETE:
                ...
                if '=' in condition:
                    ...
                    for literal in literal_list:
                        if literal in val_dict:
                            range_list.append(val_dict[literal])
                        else:
                            continue
                    single_range = NominalRange(range_list)
                    if all([single_range is None for single_range in ranges[:, attribute_index]]):
                        ranges[:, attribute_index] = single_range
                    else:
                        for i, nominal_range in enumerate(ranges[:, attribute_index]):
                ...
                if '<=' in condition:
                    ...
                    literal_list = [float(i) for i in _literal_list(condition)]
                    max_literal = max(literal_list)
                    min_literal = min(literal_list)
                    ranges = _adapt_ranges(attribute_index, max_literal, ranges, inclusive=True, lower_than=True)
                    ranges = _adapt_ranges(attribute_index, min_literal, ranges, inclusive=True, lower_than=False)
                else:
